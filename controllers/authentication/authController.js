...
exports.signup = async (req, res) => {
  ...
  if(username == '' || password == '' || email == '') return res.status(400).send({ error: 'Please fill in all fields' });

  const profanityError = checkProfanity(username, password, filter);
  ...
  const credentialsError = validateCredentials(username, password, email);
